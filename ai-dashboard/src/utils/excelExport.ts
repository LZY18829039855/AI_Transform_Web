...
export const exportTrainingDetailToExcel = (
    ...
    const deptData: Record<string, string>[] = [
      {
        ...
        '基础课程平均完课人数': intFmt(departmentRow.basicAvgCompletedCount),
        '进阶课程平均完课人数': intFmt(departmentRow.advancedAvgCompletedCount),
        '实战课程平均完课人数': intFmt(departmentRow.practicalAvgCompletedCount),
  ...
  const detailData: Record<string, string>[] = isDrillDown
    ? (detailRows as DepartmentEmployeeTrainingOverviewRow[]).map((row) => ({
        ...
        '实战目标课程数': String(row.practicalTargetCourseCount ?? 0),
        '实战目标课程完课数': String(row.practicalCompletedCount ?? 0),
        '实战目标课程完课占比': pctFmt(row.practicalCompletionRate),
